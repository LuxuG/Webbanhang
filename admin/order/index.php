...
<?php
	...
	foreach($data as $item) {
		...
		if($item['status'] == 0) {
			...
			<button onclick="changeStatus('.$item['id'].', 2)" class="btn btn-sm btn-danger">Huỷ</button>';
		} else if($item['status'] == 1) {
			echo '<label class="badge badge-success">Phê Duyệt</label>';
		} else {
			echo '<label class="badge badge-danger">Huỷ</label>';
		}
		echo '</td>
				</tr>';
	}
